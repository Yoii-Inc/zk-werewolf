...
fn load_or_generate_proving_key(
    circuit: KeyPublicizeCircuit<Fr>,
...
fn resolve_pk_path() -> Option<PathBuf> {
    let explicit = std::env::var("KEY_PUBLICIZE_GROTH16_PK_PATH").ok();
    ...
    let default_path =
        PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../zk-mpc-node/data/groth16/key_publicize_max5_v1.pk");
...
fn build_fixed_key_publicize_circuit(
    rng: &mut (impl ark_std::rand::RngCore + ark_std::rand::CryptoRng),
) -> anyhow::Result<KeyPublicizeCircuit<Fr>> {
