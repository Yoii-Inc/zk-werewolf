...
fn main() -> anyhow::Result<()> {
    ...
    // arkworks dependencies may print trace logs to stdout; keep only JSON payload.
...
fn load_or_generate_proving_key(
    circuit: RoleAssignmentCircuit<Fr>,
...
fn resolve_pk_path() -> Option<PathBuf> {
    let explicit = std::env::var("ROLE_ASSIGNMENT_GROTH16_PK_PATH").ok();
    ...
    let default_path = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("../zk-mpc-node/data/groth16/role_assignment_max5_v1.pk");
...
) -> anyhow::Result<RoleAssignmentCircuit<Fr>> {
