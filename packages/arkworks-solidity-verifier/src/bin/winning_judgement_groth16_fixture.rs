...
fn generate_fixture() -> anyhow::Result<FixtureOutput> {
    ...
    let public_inputs = vec![
        Fr::from(circuit.private_input.len() as u64),
        circuit.calculate_output(),
    ];
...
fn load_or_generate_proving_key(
    circuit: WinningJudgementCircuit<Fr>,
...
fn resolve_pk_path() -> Option<PathBuf> {
    let explicit = std::env::var("WINNING_JUDGEMENT_GROTH16_PK_PATH").ok();
    ...
    let default_path = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("../zk-mpc-node/data/groth16/winning_judgement_max5_v1.pk");
...
) -> anyhow::Result<WinningJudgementCircuit<Fr>> {
